...
class Ball(pygame.sprite.Sprite):
    ...
    def move(self, ball, racket_left, racket_right, hit_sound, goal_sound):
        ...
        self.rect.top = min(max(self.rect.top+self.speed *
                                self.direction_y, 0), config.HEIGHT-self.rect.height)
    ...
    def reset(self):
        ...
        self.rect.centery = random.randrange(
            self.rect.height//2, config.HEIGHT-self.rect.height//2)
        self.direction_x = random.choice([1, -1])
...
class Racket(pygame.sprite.Sprite):
    ...
    def move(self, direction):
        if direction == 'UP':
            ...
            raise ValueError('[direction] in Racket.move is <%s>, expect <%s> or <%s>...' % (
                direction, 'UP', 'DOWN'))
    '''电脑自动移动'''
    ...
    def reset(self):
        # 左/右边的拍
        self.rect.centerx = config.WIDTH - \
            self.rect.width//2 if self.type_ == 'RIGHT' else self.rect.width//2
